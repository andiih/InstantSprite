    ...
    sprite.mergefiles = function () {
        ...
        sprite.eachcanvas(function (canvas, i, islast) {
            ...
            if (isDiagonal) {
                ...
                var xpos = ((i) % 3);
                var ypos = Math.round(((i)/3)-0.5)
                canvas.storeX = xpos * (canvas.width + spacing);
                canvas.storeY = ypos * (canvas.width + spacing);
                totalHeight = Math.max(canvas.storeY + canvas.height, totalHeight);
                totalWidth = Math.max(canvas.storeX + canvas.width, totalWidth);
